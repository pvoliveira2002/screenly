...
import { loadCommunityServers, loadRecent, MAX_SIMULTANEOUS_SCREENS, savedName, saveCommunityServers } from './constants'
import type { ChatMessage, CommunityServer, Credentials, Member, Presentation } from './types'
import type { CommunityMessage, CommunityProfile } from './types'
import type { AccountState, AccountUser } from './components/AuthScreen'
...
export default function App() {
  ...
  const [, setRecentRooms] = useState(loadRecent)
  ...
  const [communityServers, setCommunityServers] = useState<CommunityServer[]>(loadCommunityServers)
  const [activeVoiceContext, setActiveVoiceContext] = useState<{ serverId: string; channelId: string } | null>(null)
  const [idleWarning, setIdleWarning] = useState(false)
  const [usageSeconds, setUsageSeconds] = useState(0)
  const [accountLoading, setAccountLoading] = useState(true)
  const [accountUser, setAccountUser] = useState<AccountUser | null>(null)
  const [accountMessages, setAccountMessages] = useState<CommunityMessage[]>([])
  const [accountProfile, setAccountProfile] = useState<CommunityProfile | undefined>()
  ...
  useEffect(() => {
    fetch('/api/account/session').then(async response => {
  ...
  async function persistAccountState(value: { servers?: CommunityServer[]; messages?: CommunityMessage[]; profile?: CommunityProfile }) {
    try {
      const response = await fetch('/api/account/state', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(value) })
      if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.error || 'Não foi possível salvar os dados') }
    } catch (error) { setStatus(error instanceof Error ? error.message : 'Não foi possível salvar os dados') }
  }

  async function logoutAccount() {
    if (joinedRoom) await leaveRoom()
    await fetch('/api/account/logout', { method: 'POST' })
    setAccountUser(null); setAccountMessages([]); setAccountProfile(undefined)
  }
  ...
  function saveCommunityRoom(serverId: string, channelId: string, roomCode: string) {
    ...
    changeCommunityServers(next)
  }

  async function joinCommunityVoice(serverId: string, channelId: string, participantName: string) {
    const channel = communityServers.flatMap(server => server.id === serverId ? server.categories.flatMap(category => category.channels) : []).find(item => item.id === channelId)
    if (!channel || connecting) return
    const cleanName = participantName.trim().slice(0, 32) || 'Usuário'
    setActiveVoiceContext({ serverId, channelId })
    setName(cleanName)
    localStorage.setItem('screenly-name', cleanName)
    setConnecting(true)
    setStatus('Entrando no canal de voz…')
    try {
      if (channel.roomCode) {
        try {
          const credentials: Credentials = await api('/api/token', { room: channel.roomCode, name: cleanName })
          await enterRoom(channel.roomCode, cleanName, credentials)
          return
        } catch (error) {
          const message = error instanceof Error ? error.message : ''
          if (!message.includes('encerrada') && !message.includes('Convite inválido')) throw error
        }
      }
      const credentials: Credentials = await api('/api/room', { name: cleanName })
      saveCommunityRoom(serverId, channelId, credentials.room!)
      await enterRoom(credentials.room!, cleanName, credentials)
    } catch (error) { setStatus(error instanceof Error ? error.message : 'Não foi possível entrar no canal') }
  ...
  async function enterRoom(code: string, cleanName: string, credentials: Credentials) {
    ...
    nextRoom.on(RoomEvent.DataReceived, (payload, participant, _kind, topic) => {
      ...
      try {
        const message = parseChatMessage(JSON.parse(new TextDecoder().decode(payload)))
        if (message) setChat(current => [...current.slice(-99), { ...message, sender: participant.name || participant.identity, senderId: participant.identity, local: false }])
  ...
  async function startSharing() {
    ...
    setSharingBusy(true)
    try {
      api('/api/presenter', { action: 'acquire', session_token: sessionToken.current }).catch(() => {})
      const { Track, VideoPreset } = livekit.current
      const preset = { resolution: { width: 1920, height: 1080, frameRate: 60 }, maxBitrate: 6_000_000, contentHint: 'detail' as const }
      await room.current.localParticipant.setScreenShareEnabled(
        true,
        { audio: true, resolution: preset.resolution, contentHint: preset.contentHint },
        {
          videoCodec: 'vp8',
          simulcast: true,
  ...
  async function leaveRoom() {
    ...
    resetRoom(role === 'owner' ? closeFailed ? 'Você saiu, mas não foi possível confirmar o encerramento da sala' : 'Sala encerrada' : 'Você saiu da sala')
  }
  ...
  if (accountLoading) return <main className="room room-loading">Carregando sua conta…</main>
  if (!accountUser) return <Suspense fallback={<main className="room room-loading">Abrindo login…</main>}><AuthScreen onAuthenticated={applyAccount}/></Suspense>

  if (!joinedRoom && inviteRoom) return <main className="invite-page"><nav className="topbar"><Logo/><span><ShieldCheck/> Convite de voz verificado</span></nav><section className="invite-card"><div className="invite-icon"><MonitorUp/></div><div className="eyebrow"><Radio/> CONVITE PARA CANAL DE VOZ</div><h1>Entrar na chamada</h1><p>Informe seu nome para conversar e compartilhar sua tela com a comunidade.</p><div className="invite-code"><span>CÓDIGO DO CANAL</span><strong>{inviteRoom}</strong></div><label className="invite-name">Seu nome<input autoFocus autoComplete="name" enterKeyHint="go" disabled={connecting} maxLength={32} placeholder="Como devemos chamar você?" value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!connecting&&connect(inviteRoom)}/></label><button className="main-button invite-submit" disabled={connecting} onClick={()=>connect(inviteRoom)}>{connecting?'Entrando…':'Entrar no canal'}</button><small aria-live="polite">{status}</small></section><footer className="invite-footer"><ShieldCheck/> Convite validado pelo servidor</footer></main>

  const activeVoiceServer = communityServers.find(server => server.id === activeVoiceContext?.serverId)
  const activeVoiceChannel = activeVoiceServer?.categories.flatMap(category => category.channels).find(channel => channel.id === activeVoiceContext?.channelId)
  ...
  const voiceView = joinedRoom ? <Suspense fallback={<main className="room room-loading">Preparando o canal…</main>}><main className={`room embedded-room ${panelOpen?'':'panel-closed'}`}>
    ...
    <div className="room-body"><section className={`stage ${presentations.length?'multi-stage':''} ${focusedScreen?'focus-mode':''}`} ref={stage} onClick={()=>audioBlocked&&enableAudio()}><div ref={audioRoot} className="audio-root"/>{presentations.length===0&&<div className="empty-stage"><div className="empty-visual"><MonitorUp/></div><span>SALA PRONTA</span><h2>Compartilhe quando quiser</h2><p>Todos podem compartilhar telas ao mesmo tempo, sempre na melhor qualidade disponível.</p><button className="main-button" disabled={sharingBusy} onClick={startSharing}><MonitorUp/> {sharingBusy?'Abrindo seletor…':'Compartilhar minha tela'}</button></div>}{presentations.map(item=><ScreenTile key={item.id} presentation={item} focused={focusedScreen===item.id} onFocus={()=>setFocusedScreen(current=>current===item.id?'':item.id)}/>)}{audioBlocked&&<button className="audio-overlay" onClick={enableAudio}><Volume2/> Clique para ativar o áudio</button>}</section>
  ...
  return <Suspense fallback={<main className="room room-loading">Abrindo seus servidores…</main>}><CommunityHub key={accountUser.id} name={name} currentUserId={accountUser.id} servers={communityServers} initialMessages={accountMessages} initialProfile={accountProfile} onPersist={persistAccountState} onSaveProfile={saveAccountProfile} onMessageAction={communityMessageAction} onLogout={logoutAccount} onChange={changeCommunityServers} onInvite={createCommunityInvite} onJoinVoice={joinCommunityVoice} activeVoiceServerId={activeVoiceContext?.serverId} activeVoiceChannelId={activeVoiceContext?.channelId} voiceMembers={members} voiceView={voiceView} voicePanel={joinedRoom ? voicePanel : undefined} friendsView={<FriendsPanel/>}/></Suspense>
}

function Logo(){return <div className="logo"><span>S</span><strong>Screenly</strong></div>}

function parseChatMessage(value: unknown): Pick<ChatMessage, 'id' | 'text' | 'time'> | null {
  if (!value || typeof value !== 'object') return null
  const message = value as Record<string, unknown>
  if (typeof message.id !== 'string' || !message.id || message.id.length > 100) return null
  if (typeof message.text !== 'string' || !message.text.trim() || message.text.length > 1000) return null
  if (typeof message.time !== 'number' || !Number.isFinite(message.time)) return null
  return { id: message.id, text: message.text, time: message.time }
}
