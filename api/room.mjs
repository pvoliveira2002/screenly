...
export default async function handler(req, res) {
  ...
  try {
    ...
    const message = error?.message === 'CONFIG_MISSING' ? 'LiveKit não configurado' : error?.message === 'INVALID_JSON' ? 'Dados da solicitação inválidos' : 'Não foi possível criar a sala'
